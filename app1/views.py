...
def updateLeaderboard(request):
    question_solved_by_user = json.loads(
        request.POST.getlist('question_solved_by_user')[0])

    for user in question_solved_by_user.keys():
        obj = Users.objects.filter(usr_name=user).update(
            solved_questions=question_solved_by_user[user])

    data = request.POST.getlist('score')
    user_handle = request.POST.getlist('handle')
    question_user_count = json.loads(
        request.POST.getlist('question_user_count')[0])
    print(question_user_count)
    j = 0
    Leaderboard.objects.all().delete()
    for i in range(len(data)):
        l = Leaderboard(usr_name=user_handle[i], score=int(
            data[i]), profile_pic=Users.objects.get(usr_name=user_handle[i]).profile_pic)
        l.save()
